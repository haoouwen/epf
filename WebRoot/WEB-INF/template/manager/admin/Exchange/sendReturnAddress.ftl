...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut" onclick="sendAddressToMember('${(exchange.trade_id)?if_exists}','buyeraddr.addr_id','/admin_Exchange_sendAddressToMember.action')" value="发送收货地址">
     <input type="button" class="rbut" name="returnList" value="返回列表" onclick="linkToInfo('/admin_Exchange_exGoodsList.action','');"/>
   </div>
<!--按钮操作存放结束-->
</div>
<!--表单框隐藏域存放-->

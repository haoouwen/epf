<html>
  ...
  <body>
<@s.form action="/admin_Shiptemplate_insert.action" method="post" validate="true" id="shipfrom">
<!--当前位置开始-->
<div class="postion">
当前位置:商城管理 > 物流管理 > 添加国内运费模版
</div>
<!--当前位置结束-->
<div class="rtdcont">
	<div class="rdtable">
						    			...
						    			<#assign s_start=0/>
								    	  ...
								    	  <#if (sm.smode_id?if_exists==ship.c_mid?if_exists) && ship.d_ship=='0'>
							    	   		   <div class="defalut_mode">
									    			...
									    			<span>默认运费：<input  name='first_weight' type="text" value="${ship.f_weight?if_exists}"/> <font class='fd_unit'>件</font>内，</span>
									    			<span><input  name='first_price' type="text" value="${ship.f_price?if_exists}"/> 元，</span>
									    			<span>每增加<input name='cont_weight' type="text" value="${ship.c_weight?if_exists}"/> <font class='fd_price'>件</font>，</span>
   ...
   <div class="bsbut_detail">
	 <@s.token/>    
	 <!--所属地区插件隐藏字段开始区域-->
	 <@s.hidden name="custId" value="0"/>
	 <@s.hidden id="hidden_area_value" name="hidden_area_value"/>
	 ${listSearchHiddenField?if_exists}
	 <input type="button" value="保存" onclick="shipsubmit();"/>
	 <@s.reset value="重置"/>
	 <input type="button" name="returnList" value="返回列表" onclick="linkToInfo('/admin_Shiptemplate_list.action','custId=${custId?if_exists}');"/>

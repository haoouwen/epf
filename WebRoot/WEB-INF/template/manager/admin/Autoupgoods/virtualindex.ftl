...
<body>
<@s.form action="/admin_Autoupgoods_virtuallist.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置：商品管理 > 虚拟商品管理 > 虚拟商品上架管理列表</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
 ...
 <#list autoupgoodsList as autoupgoods>
  <tr>
    <td align="center"><input type="checkbox" name="autoupgoods.trade_id" value="${autoupgoods.trade_id?if_exists}"/></td>    
    	...
    	<td align="center">
			<#if autoupgoods.cust_name?if_exists!=null && autoupgoods.cust_name?if_exists!=''>
				${autoupgoods.cust_name?if_exists}
			<#else>
			 	-
			</#if>
...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut" onclick="delInfo('autoupgoods.trade_id','/admin_Autoupgoods_bmalldelete.action')" value="删除">
   </div>
<!--按钮操作存放结束-->
</div>
<!--表单框隐藏域存放-->
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Autoupgoods_virtuallist.action" method="post"  id="form_search_id">

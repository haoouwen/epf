<html>
  ...
  <body>
<@s.form action="/admin_Goods_couponGoodsList.action" method="post"  id="indexForm">
<@s.hidden name="goods.is_up" id="admin_goods_is_ip"/>
<@s.hidden  id="admin_goods_tab"/>
<!--当前位置-->
	<div class="postion">当前位置：营销推广 > 促销管理 > 优惠券商品</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
	  ...
	  <#list goodsList as goods>
	    <tr >
		    ...
		    <td style="text-align:left;padding-left:10px;line-height:20px">
		    	...
		    	<#if goods.coupon_name?if_exists !=null && goods.coupon_name?if_exists !="">
		    	 <br/>
		    	 <#list goods.coupon_name?split(',') as coupon_name>
		    	 <font color="red">[${coupon_name?if_exists}] </font>
		    	 </#list>
		    	 </#if>
		    </td>
		    <td align="center">

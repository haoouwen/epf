...
<body>
<@s.form action="/admin_Virtualgoods_auditlist.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置:商品管理 > 虚拟商品管理 > 虚拟商品审核列表</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
    ...
    <td><input type="checkbox" name="goods.goods_id" value="${goods.goods_id?if_exists}"/></td>    
    	...
    	<#if goods.goods_no?if_exists!=null && goods.goods_no?if_exists!=''>
	      ...
	      <#if goods.goods_name?if_exists?length lt 30>
	      	<a onclick="linkToInfo('/admin_Virtualgoods_audit.action','goods.goods_id=${(goods.goods_id)?if_exists}');" title="${goods.goods_name?if_exists}">${goods.goods_name?if_exists} </a>
		 <#else>
		 	<a onclick="linkToInfo('/admin_Virtualgoods_audit.action','goods.goods_id=${(goods.goods_id)?if_exists}');" title="${goods.goods_name?if_exists}">${goods.goods_name?if_exists[0..29]}</a>
		 	..
		 </#if>
        ...
        <td align="center"><a onclick="linkToInfo('/admin_Virtualgoods_audit.action','goods.goods_id=${goods.goods_id?if_exists}');"><img src="/include/common/images/audit.png" /></a></td>
  </tr>
  </#list>
	</table>
  </div>
...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut" onclick="delInfo('goods.goods_id','/admin_Virtualgoods_auditelete.action')" value="删除">
   </div>
<!--按钮操作存放结束-->
</div>
<!--表单框隐藏域存放-->
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Virtualgoods_auditlist.action" method="post"  id="form_search_id">

...
<body>
<@s.form action="/admin_Virtualgoods_storelist.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion"> 	当前位置:商品管理 > 虚拟商品管理 > 仓库中虚拟商品列表</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
...
<!--后台table-->
  ...
  <tr>
    <td><input type="checkbox" name="goods.goods_id" value="${goods.goods_id?if_exists}"/></td>    
    	...
    	<td align="center">
	    	<#if goods.brand_name?if_exists!=null && goods.brand_name?if_exists!=''>
		    		${goods.brand_name?if_exists}
		    	<#else>
		    	 	-
	    	</#if>
    	</td>
        ...
        <td align="center"><a onclick="linkToInfo('/admin_Virtualgoods_storeview.action','goods.goods_id=${goods.goods_id?if_exists}');"><img src="/include/common/images/bj.gif" /></a></td>
  </tr>
  </#list>
	</table>
  </div>
...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut" onclick="updateStateup('1','goods.goods_id','/admin_Virtualgoods_storeup.action');" value="上架">
     <input type="button" class="rbut" onclick="delInfo('goods.goods_id','/admin_Virtualgoods_storedelete.action')" value="删除">
   </div>
<!--按钮操作存放结束-->
</div>
<!--表单框隐藏域存放-->
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Virtualgoods_storelist.action" method="post"  id="form_search_id">
		...
		<tr>
			...
			<tr>
				<td class="searchDiv_td">所属分类:</td> 
						<td>
					             <@s.hidden type="hidden" id="cat_attr_s" name="cat_attr_s"/>

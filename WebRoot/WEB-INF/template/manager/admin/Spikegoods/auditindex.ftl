...
<body>
<@s.form action="/admin_Spikegoods_auditList.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置:营销推广 > 秒杀商品 > 秒杀商品审核</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
...
<!--后台table-->
  ...
  <tr>
    <td><input type="checkbox" name="spikegoods.trade_id" value="${spikegoods.trade_id?if_exists}"/></td>    
    	...
    	<td align="center">
    		<#if spikegoods.user_name?if_exists!=''&& spikegoods.user_name?if_exists!=null>
    			${spikegoods.user_name?if_exists}
	    	<#else>
	    	-
	    	</#if>
    	</td>
    ...
    <td align="center"><a onclick="linkToInfo('/admin_Spikegoods_audit.action','spikegoods.trade_id=${spikegoods.trade_id?if_exists}');"><img src="/include/common/images/bj.gif" /></a></td>
  </tr>
  </#list>
	</table>
  </div>
...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut" onclick="updateSort('spikegoods.trade_id','isort_no','/admin_Spikegoods_auditUpdateSort.action');" value="排序">
     <input type="button" class="rbut" onclick="delInfo('spikegoods.trade_id','/admin_Spikegoods_auditDelete.action')" value="删除">
   </div>
<!--按钮操作存放结束-->
</div>
<!--表单框隐藏域存放-->
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Spikegoods_auditList.action" method="post"  id="form_search_id">

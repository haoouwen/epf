<html>
  ...
  <body>
<@s.form action="/admin_Salegoods_list.action" method="post" id="indexForm">

        <div class="postion">当前位置：营销推广 > 促销管理 > 商品促销列表</div>
        <div class="rtdcont">
           <!--条件查询-->
           <div class="rseacher">
           ...
           <div class="rtable">
             <table width="100%" cellspacing="0" cellpadding="0" class="indexTab">
               <tr>
                 <th width="5%"><input type="checkbox" name="checkbox" id="checkall" onclick="selectAll('salegoods.sale_id')"/></th>
			    ...
			    <#list salegoodsList as salegoods>
				 <tr >
			         <td><input type="checkbox" name="salegoods.sale_id" value="${salegoods.sale_id?if_exists}"/></td>
			         
			         	<td align="center">${salegoods.sale_name?if_exists}</td>
				    
			         	<td align="center"><#if salegoods.info_state=="1"><font color='green'>启用<#else><font color='red'>禁用</#if></td>
				    
			         	<td align="center">${salegoods.priority?if_exists}</td>
			         	
			         	<td align="center">
			         	<#list salegoods.platform?trim?split(',') as platform>
			         	   <#if platform?trim=='0'>
			         	     PC端
			         	...
			         	<td align="center"><#if salegoods.is_recome=="1"><font color='green'>是<#else><font color='red'>否</#if></td>
				    
			         	<td align="center">${salegoods.start_time?if_exists}</td>
				    
			         	<td align="center">${salegoods.end_time?if_exists}</td>
				    
			         	<td align="center">${salegoods.member_level?if_exists}</td>
				    
				    <td align="center"><a onclick="linkToInfo('/admin_Salegoods_view.action','salegoods.sale_id=${salegoods.sale_id?if_exists}');">
           ...
           <!--翻页-->
           <div class="bsbut">
             <input type="button" class="rbut" onclick="linkToInfo('/admin_Salegoods_add.action','');" value="添加商品促销">
             <input type="button" class="rbut" onclick="updateBatchState('1','salegoods.sale_id','/admin_Salegoods_updateStateB2C.action','启用');" value="启用">             
			 <input type="button" class="rbut" onclick="updateBatchState('0','salegoods.sale_id','/admin_Salegoods_updateStateB2C.action','禁用');" value="禁用">  
             <input type="button" class="rbut" onclick="updateBatchState('1','salegoods.sale_id','/admin_Salegoods_updateIsrecome.action','排它');" value="排它">             
			 <input type="button" class="rbut" onclick="updateBatchState('0','salegoods.sale_id','/admin_Salegoods_updateIsrecome.action','取消排它');" value="取消排它">  			 
             <input type="button" class="rbut"onclick="delInfo('salegoods.sale_id','/admin_Salegoods_delete.action')" value="删除">
           </div>
        </div>
		<!--表单框隐藏域存放-->
...
<@s.form action="/admin_Salegoods_list.action"   method="post"  id="form_search_id">
<!--dd-->
	<table>
	<tr>

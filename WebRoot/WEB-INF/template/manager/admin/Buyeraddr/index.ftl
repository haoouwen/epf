...
<body>
<@s.form action="/admin_Buyeraddr_list.action" method="post"  id="indexForm">
<!--当前位置-->
     ...
     <table width="100%" cellspacing="0" cellpadding="0" class="indexTab">
      <tr >
	         <th width="3%" ><input type="checkbox" name="checkbox" id="checkall" onclick="selectAll('buyeraddr.addr_id')"/></th>
	     	 ...
	     	 <th width="10%"  align="center" >省/市区串</th>
	    
	         <th width="10%"  align="center" >邮政编码</th>
	 ...
	 <#list buyeraddrList as buyeraddr>
	  <tr>
	    <td><input type="checkbox" name="buyeraddr.addr_id" value="${buyeraddr.addr_id?if_exists}"/></td>    
	    	...
	    	<td align="center">
	    	<#if buyeraddr.user_name?if_exists!=null && buyeraddr.user_name?if_exists!=''>
		    		${buyeraddr.user_name?if_exists}
		    	<#else>
		    	 	-
		    	</#if>
	    	</td>
	    ...
	    <td align="center"><a onclick="linkToInfo('/admin_Buyeraddr_view.action','buyeraddr.addr_id=${buyeraddr.addr_id?if_exists}');"><img src="/include/common/images/bj.gif" /></a></td>
	  </tr>
	  </#list>
	</table>
  </div>
...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut" onclick="delInfo('buyeraddr.addr_id','/admin_Buyeraddr_delete.action')" value="删除">
   </div>
<!--按钮操作存放结束-->
</div>
<!--表单框隐藏域存放-->
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Buyeraddr_list.action" method="post"  id="form_search_id">

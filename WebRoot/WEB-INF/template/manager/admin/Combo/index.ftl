...
<body>
<@s.form action="/admin_Combo_list.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置：营销推广 > 套餐商品 > 套餐列表</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
      ...
      <tr >
		...
		<#list comboList as combo>
		  <tr>
		    ...
		    <td align="center"><a onclick="linkToInfo('/admin_Combo_view.action','combo.trade_id=${combo.trade_id?if_exists}');"><img src="/include/common/images/bj.gif" /></a></td>
		  </tr>
		  </#list>
	</table>
  </div>
...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut" onclick="linkToInfo('/admin_Combo_add.action','');" value="添加">
     <input type="button" class="rbut" onclick="updateSort('combo.trade_id','isort_no','/admin_Combo_updateSort.action');" value="排序">
     <input type="button" class="rbut" onclick="delInfo('combo.trade_id','/admin_Combo_delete.action')" value="删除">
   </div>
<!--按钮操作存放结束-->
</div>
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Combo_list.action" method="post"  id="form_search_id">

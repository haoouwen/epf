<html>
  ...
  <body>
<@s.form action="/admin_Goodsorder_allorderlist.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置：商城管理 > 订单管理 > 全部订单</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
	     <table cellpadding="0" cellspacing="0">
	      <tr>
	       ...
	       <td class="tdpad"><input type="submit" class="button button-primary button-rounded button-tiny"  class="rbut" value="查询"></td>
	       <td class="tdpad"><input type="button"  class="button button-primary button-rounded button-tiny"   onclick="searchShowDIV('searchDiv','300px','220px');" class="rbut" value="高级查询"></td>
	       <td class="tdpad"><input type="button" class="button button-primary button-rounded button-tiny"  onclick="recycle('goodsorder.order_id','/admin_Goodsorder_recycle.action');" class="rbut" value="订单回收"></td>
	       <td class="tdpad"><button type="button" class="button button-primary button-rounded button-tiny"  onclick="export_order('prepare_goods');">导出备货清单</button></td>
	       <td class="tdpad"><button type="button" class="button button-primary button-rounded button-tiny" onclick="export_order('deliver_goods');">导出发货清单</button></td>
	   </tr>
	  </table>
	</div>
<!--条件结束-->
<!--后台table-->
       ...
       <div class="orderDiv" id="orderDivId">
                ...
                <div class="orderH2">
                  <ul>
                    <li <#if order_state_s=='999'||order_state_s==''>class="selli"</#if>><a href="/admin_Goodsorder_allorderlist.action?order_state_s=999">全部订单</a></li>
                     <#list commparaList as clist>
                    <li <#if order_state_s=='${clist.para_value?if_exists}'>class="selli"</#if>><a href="/admin_Goodsorder_allorderlist.action?order_state_s=${clist.para_value?if_exists}">${clist.para_key?if_exists}</a></li>
                    </#list>
                                  ...
                                  <div class="tddiv">
                                      <p class="pftd">
                                         <span><input type="checkbox" name="goodsorder.order_id" value="${goodsorder.order_id?if_exists}"/></span>
                                          ...
                                          <#if goodsorder.customs_name?if_exists!=""><span>海关类型：<font color="#005ea7">${goodsorder.customs_name}</font></span></#if>
                                          <#if goodsorder.sosysno?if_exists!=""><span>海关订单编号：<font color="#005ea7">${goodsorder.sosysno}</font></span></#if>
										   <span>状态：
                                          ...
                                          <#if goodsorder.order_state=='4'>
                                           ...
                                           <#list paymentList as plist>
                                            ...
                                            <#if pay_code=="alipay" || pay_code=="alipaywap">
                                              <input type="button" onclick="alipayRefund('','${goodsorder.order_id?if_exists}','${(goodsorder.tatal_amount)?if_exists}','${(goodsorder.order_time)?if_exists}','${(goodsorder.consignee)?if_exists}','${(goodsorder.area_attr)?if_exists}  ${(goodsorder.buy_address)?if_exists}','${(goodsorder.mobile)?if_exists}','${(goodsorder.pay_name)?if_exists}','${(goodsorder.pay_id)?if_exists}','${(goodsorder.pay_time)?if_exists}','${(goodsorder.order_state)?if_exists}','${(goodsorder.pay_code)?if_exists}','${(goodsorder.pay_trxid)?if_exists}','operatorsorderindex');" title="支付宝退款" value="支付宝退款" class="cssbtn"/>
                                            </#if>
                                           </#if>
                                       </p>
                                  ...
                                  <div class="tddivCont">
                                     <table width="100%" cellspacing="0" cellpadding="0" class="orderTab1">
                                       <tr>
														  ...
														  <#if ((detail.order_id)?if_exists)==((goodsorder.order_id)?if_exists)>
				                                              ...
				                                              <tr>
				                                                ...
				                                                <td width="15%" class="gray">
				                                         		...
				                                         		<td width="16%"><b>${(detail.goods_price)?if_exists}</b></td>
									    ...
									    <a onclick="linkToInfo('/admin_Goodsorder_orderDetail.action','goodsorder.order_id=${goodsorder.order_id?if_exists}&order_state=${(goodsorder.order_state)?if_exists}');" >订单详情</a>
										</td>
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Goodsorder_allorderlist.action" method="post"  id="form_search_id">
...
<#include "/WEB-INF/template/manager/admin/Goodsorder/allcancelOrderTip.ftl">
<!-- 打印票据 end-->
</body>

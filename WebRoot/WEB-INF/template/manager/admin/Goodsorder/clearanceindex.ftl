<html>
 ...
 <body>
<@s.form action="/admin_Goodsorder_clearancelist.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置：商城管理 > 直邮发货 > 订单通关</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
	     <table cellpadding="0" cellspacing="0">
	      <tr>
	       ...
	       <td>  <input type="button" class="rbut" onclick="batchclearanceShowDIV('batchclearance');"  value="批量通关" title="批量通关操作"></td>
	   </tr>
	  </table>
	</div>
<!--条件结束-->
                ...
                <div class="tabDiv">
                     ...
                     <div class="orderTab" id="oftabId">
                        <table width="100%" cellspacing="0" cellpadding="0">
                            <tr>
                            <th width="3%"><input type="checkbox" name="checkbox" id="checkall" onclick="selectAll('goodsorder.order_id')"/></th>
                              ...
                              <td colspan="9">
                                  <div class="tddiv">
                                      <p class="pftd90">
                                        ...
                                        <span>订单编号：<b><a onclick="linkToInfo('/admin_Goodsorder_view.action','goodsorder.order_id=${goodsorder.order_id?if_exists}&order_state=${(goodsorder.order_state)?if_exists}&detail_return_flage=11');" >${(goodsorder.order_id)?if_exists}</a></b></span>
										   <span>状态：
                                         ...
                                         <#if (goodsorder.is_clearance)?if_exists=='0'>
                                        <p class="prtd">
                                         <span> <input type="button" 
                                            onclick="clearanceShowDIV('clearance','${goodsorder.order_id?if_exists}','${(goodsorder.tatal_amount)?if_exists}','${(goodsorder.order_time)?if_exists}','${(goodsorder.consignee)?if_exists}','${(goodsorder.area_attr)?if_exists}  ${(goodsorder.buy_address)?if_exists}','${(goodsorder.mobile)?if_exists}');"
                                           title="设置通关状态" value="订单通关" class="cssbtn"/></span>
                                       </p>
                                       </#if>
                                  ...
                                  <div class="tddivCont">
                                     ...
                                     <table width="100%" cellspacing="0" cellpadding="0" class="orderTab1">
                                       <tr>
									    ...
									    <a onclick="linkToInfo('/admin_Goodsorder_view.action','goodsorder.order_id=${goodsorder.order_id?if_exists}&order_state=${(goodsorder.order_state)?if_exists}&detail_return_flage=11');" >订单详情</a>
										</td>
...
<!-- 打印运单 end-->
<!-- 修改通关状态-->
<div style="display:none;"  id="clearance" >
      <@s.form  action="/admin_Goodsorder_updateClearance.action"  method="post"  >
	        <table>
	            <tr>
	           	<td  class="table_name">订单信息：</td>
           		<td id="tgorderinfo_id"></td>
	          </tr>
	          <tr>
	           	<td  class="table_name">收货人信息：</td>
           		<td id="tgshouhuoren_id"></td>
	          </tr>
	           <tr>
	           	<td  class="table_name">通关状态：</td>
           		<td> <@s.radio name="oidis_clearance"  list=r"#{'1':'通关成功','2':'通关失败'}" /></td>
	          </tr>
	          <tr>
	           <td  colspan="2" align="center"><br/>
	             <@s.hidden name="clearance_oid" id="clearance_oid" />
	           	<@s.submit value="提交通关" cssClass="submitbut" />
	           	<input type="button" name="close" value="关闭" onclick="closeSearchShowDIV('clearance')"/>
	           </td>
	          </tr> 
	       </table>
	     </@s.form>
</div>
<div  style="display:none;" id="batchclearance" >
      <@s.form  action="/admin_Goodsorder_batchClearance.action"  method="post" id="printDeliveryForm" class="searchDiv">
	        <table>
	            <tr>
	           	<td  class="table_name">系统提示:</td>
           		<td id="batch_clearance_id"></td>
	          </tr>
	           <tr>
	           	<td  class="table_name">通关状态：</td>
           		<td> <@s.radio name="oidis_clearance1" id="tgrid" list=r"#{'1':'通关成功','2':'通关失败'}"  /></td>
	          </tr>
	          <tr>
	           <td  colspan="2" align="center"><br/>
	             <@s.hidden name="clearance_oid_str" id="order_id_str"/>
	               <@s.hidden id="selectnumber"/>
	            <input type="button" class="rbut" value="订单通关" onclick="batchclearanceshow();"/>
	           	<input type="button" class="rbut" name="close" value="关闭" onclick="closeSearchShowDIV('batchclearance')"/>
	           </td>
	          </tr> 
	       </table>
	     </@s.form>

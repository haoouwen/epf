<html>
  ...
  <body>
<@s.form action="/admin_Goodsorder_waitconfirmdeliver.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置：商城管理 > 直邮发货 > 确认发货</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
	     <table cellpadding="0" cellspacing="0">
	      <tr>
	        ...
	        <td>  <input type="button" class="rbut" onclick="order_print_fahuo_all('2');"  value="补打配货单" title="批量重新补打印配货单"></td>
	        <td>  <input type="button" class="rbut" onclick="batchpintExpressShowDIV('batchprintepress','2');"  value="补打运单" title="批量重新补打印运单"></td>
	        <td><input type="button" onclick="recycle('goodsorder.order_id','/admin_Goodsorder_conformSendRecycle.action');" class="rbut" value="订单回收"></td>
	   </tr>
	  </table>
	</div>
<!--条件结束-->
                              ...
                              <td colspan="9">
                                  <div class="tddiv">
                                      <p class="pftd">
                                          ...
                                          <span>订单编号：<b><a onclick="linkToInfo('/admin_Goodsorder_view.action','goodsorder.order_id=${goodsorder.order_id?if_exists}&order_state=${(goodsorder.order_state)?if_exists}&detail_return_flage=6');" >${(goodsorder.order_id)?if_exists}</a></b></span>
                                        ...
                                        <span>
									       ...
									       <#if goodsorder.print_kuaidi?if_exists=="0">
									        <font color="red" title="运单已打印">[递]<#if (goodsorder.print_kuaidi_number?if_exists gt 0)>[补${goodsorder.print_kuaidi_number}]</#if></font>
									        </#if>
                                          </span>
                                       </p>
                                       <p class="prtd">
                                           <input type="button" 
                                           onclick="fahuoShowDIV('invoice','${goodsorder.order_id?if_exists}','${(goodsorder.tatal_amount)?if_exists}','${(goodsorder.order_time)?if_exists}','${(goodsorder.consignee)?if_exists}','${(goodsorder.area_attr)?if_exists}  ${(goodsorder.buy_address)?if_exists}','${(goodsorder.mobile)?if_exists}','${(goodsorder.print_smode_no)?if_exists}','${(goodsorder.send_mode)?if_exists}','${(goodsorder.send_num)?if_exists}');"
                                           title="点击提交确认发货" value="确认发货" class="cssbtn"/>
                                           <input type="button"   
                                            onclick="cancelOrderShowDIV('cancelorderinfo','${goodsorder.order_id?if_exists}','${(goodsorder.tatal_amount)?if_exists}','${(goodsorder.order_time)?if_exists}','${(goodsorder.consignee)?if_exists}','${(goodsorder.area_attr)?if_exists}  ${(goodsorder.buy_address)?if_exists}','${(goodsorder.mobile)?if_exists}','${(goodsorder.pay_name)?if_exists}','${(goodsorder.pay_id)?if_exists}','${(goodsorder.pay_time)?if_exists}','${(goodsorder.order_state)?if_exists}','${(goodsorder.pay_code)?if_exists}','${(goodsorder.pay_trxid)?if_exists}','waitconfirmdeliver');"
                                       ...
                                       <tr>
										...
										<td  width="9%"  class="gray" rowspan="9">
                                         ...
                                         <td width="12%"   rowspan="10">	
										        	<p>
	                                               <a onclick="linkToInfo('/admin_Goodsorder_view.action','goodsorder.order_id=${goodsorder.order_id?if_exists}&order_state=${(goodsorder.order_state)?if_exists}&detail_return_flage=6');" >订单详情</a>
	                                             </p>
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Goodsorder_waitconfirmdeliver.action" method="post"  id="form_search_id">
...
<div  style="display:none;" id="invoice" >
      <@s.form id="DeliveryForm" action="/admin_Goodsorder_confirmDeliveryGoodsOrder.action"  method="post" class="searchDiv" >
	        <table >
	         <tr>
	           	<td  class="table_name" width="30%">订单信息：</td>
	          ...
	          <tr>
	            ...
	            <tr>
           		...
           		<td> <@s.select list="sendmodeList" listKey="smode_id" onchange="loadsendmodelenth('sendmode_id','order_send_num_id');" listValue="smode_name" name="order_send_mode" style="width:120px" id="sendmode_id"/></td>
	          </tr>
	           <tr>
	          ...
	          <tr>
	           <td  colspan="2" align="center"><br/>
	             ...
	             <@s.hidden name="is_modify" value="0" />
	           	<input type="button" onclick="postConfirmDelivery()" value="确认发货"/>

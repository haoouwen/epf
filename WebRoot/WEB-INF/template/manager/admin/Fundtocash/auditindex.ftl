<html>
  ...
  <body>
<@s.form action="/admin_Fundtocash_auditList.action" method="post"  id="indexForm">
<!--当前位置-->
	<div class="postion">当前位置： 财务统计 > 财务管理 > 提现审核</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
     ...
     <table width="100%" cellspacing="0" cellpadding="0" class="indexTab">
	  ...
	  <#list fundtocashList as fundtocash>
		  <tr>
		    ...
		    <td align="center"><#if fundtocash.order_state?if_exists=='0'><a onclick="linkToInfo('/admin_Fundtocash_list.action','order_state=${fundtocash.order_state?if_exists}');"><font color='red'>未审核</font></a></#if>
		    <#if fundtocash.order_state?if_exists=='2'><a onclick="linkToInfo('/admin_Fundtocash_list.action','order_state=${fundtocash.order_state?if_exists}');"><font color='blue'>未通过</font></a></#if>
		    <td align="center">${fundtocash.in_date?if_exists}</td>
		    <td align="center"><a onclick="linkToInfo('/admin_Fundtocash_audit.action','fundtocash.trade_id=${fundtocash.trade_id?if_exists}');"><img src="/include/common/images/bj.gif" /></a></td>
		  </tr>
	  </#list>
	</table>
  </div>
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Fundtocash_auditList.action" method="post"  id="form_search_id">
		...
		<tr>
			...
			<td><@s.select name="order_state" list=r"#{'0':'未审核','2':'未通过'}" headerKey="" headerValue="请选择"/>  </td>
		</tr>
		<tr>

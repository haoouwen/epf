<html>
  ...
  <body>
<@s.form action="/admin_Groupgoods_auditList.action" method="post"  id="indexForm">
<@s.hidden name="groupgoods_sortno_id" id="groupgoods_sortno_id"/>


<!--当前位置-->
	<div class="postion">当前位置：营销推广 > 团购商品 > 团购商品审核</div>
<!--当前位置结束-->
<!--条件查询-->
	<div class="rtdcont">
	   <div class="rseacher">
...
<!--后台table-->
   <div class="rtable">
     <table width="100%" cellspacing="0" cellpadding="0" class="indexTab">
          <tr >
		    <th width="3%" > <input type="checkbox" name="checkbox" id="checkall" onclick="selectAll('groupgoods.trade_id')"/></th>
		    <th width="5%"  >排序</th>
		    <th width="22%"  >团购标题</th>
		    <th width="13%"  >开始时间</th>
		    <th width="13%"  >结束时间</th>
	   ...
	   <#list groupgoodsList as groupgoods>
	    <tr >
		    <td><input type="checkbox" name="groupgoods.trade_id" value="${groupgoods.trade_id?if_exists}"/></td>
		    <td align="center">
		     ...
		     <td align="center">
				<a onclick="linkToInfo('/admin_Groupgoods_audit.action','groupgoods.trade_id=${groupgoods.trade_id?if_exists}');"><img src="/include/common/images/bj.gif" /></a>
	  </tr>
	  </#list>
	</table>
  </div>
...
<!--按钮操作存放-->
   <div class="bsbut">
     <input type="button" class="rbut"onclick="updateSort('groupgoods.trade_id','isort_no','/admin_Groupgoods_auditUpdateSort.action');" value="排序">
     <input type="button" class="rbut"onclick="delInfo('groupgoods.trade_id','/admin_Groupgoods_auditDelete.action')" value="删除">
   </div>
<!--按钮操作存放结束-->
</div>
<!--表单框隐藏域存放-->
...
<!--搜索区域开始-->
	...
	<@s.form action="/admin_Groupgoods_auditList.action" method="post"  id="form_search_id">
		...
		<tr>
			...
			<td>
				<select name="info_state_s" style="width:108px;">
					<option value="" selected>请选择
					<#list infoStateList as infoState>
						<#if infoState.para_value?if_exists='0'>
					       <option value="${infoState.para_value?if_exists}" <#if info_state_s?if_exists==infoState.para_value?if_exists>selected</#if>>${infoState.para_key?if_exists}

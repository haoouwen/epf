<html>
  ...
  <body>
<@s.form action="/admin_Role_update.action" method="post" validate="true" id="modiy_form">
<!--当前位置开始-->
<div class="postion">
...
<div class="rtdcont">
             ...
             <td id="sysmenuTab">
                <div class="tabbar" >
				       ...
				       <#list menuoneList as menuone>
						   	<li  <#if menuone_index==0> class="selected"</#if>><span class="chk"><input type="checkbox" class="menu_right" name="role.menu_right" id="${menuone.menu_id?if_exists}"  value="${menuone.menu_id?if_exists}" /></span>${(menuone.menu_name)?if_exists}</li>
					   </#list>
				     </ul>
		       </div>
	             ...
	             <div class="tabdiv"  <#if menuone_index==0>style="display:block;"</#if>>
		             <#list menutowList as menutow>
				         	 <#if menuone.menu_id==menutow.up_menu_id>
				         	 	<div class="towleve">
		                          	  <div class="towleve_one_menu"><b><input type="checkbox"  class="menu_right"  name="role.menu_right" id="${menuone.menu_id?if_exists}${menutow.menu_id?if_exists}" value="${menutow.menu_id?if_exists}"/>${menutow.menu_name?if_exists}:</b></div>
					                            ...
					                            <div class="threeleve">
						                            <ul>
						                                <li>
						                                	<table>
						                                		<tr>
						                                			<td  width="145px"><b><input  class="menu_right" type="checkbox" value="${menuthree.menu_id?if_exists}" name="role.menu_right" id="${menuone.menu_id?if_exists}${menutow.menu_id?if_exists}${menuthree.menu_id?if_exists}"/>${menuthree.menu_name?if_exists}</b></td>
						                                			<td>
						                                			<#assign x=0>	
						                                			<#list rolerightList as roleright>
									                                	<#if menuthree.menu_id==roleright.menu_attr>
									                                		<#assign x=x+1>
									                                		<span><input id="${menuone.menu_id?if_exists},${menutow.menu_id?if_exists},${menuthree.menu_id?if_exists},${roleright.right_id?if_exists}" class="oper_right" type="checkbox" name="role.oper_right" value="${roleright.right_id?if_exists}"/>${roleright.right_name?if_exists}</span>
									                                		<#if x%7==0>
									                                			<br/>
									                                		</#if>
									                                	</#if>
   ...
   <div class="bsbut_detail">
       ...
       <@s.hidden name="oldRoleName" value="${role.role_name}"/>
       <@s.hidden id="menu_right" value="${(role.menu_right)?if_exists}"/>
       <@s.hidden id="oper_right" value="${(role.oper_right)?if_exists}"/>

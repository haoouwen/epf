...
<@s.form action="/agent_Asysuser_dailigoods.action" method="post" id="indexForm" name="indexForm">
            ...
            <div class="search-content">
                <form action="#" method="post">
                    <table class="search-tab">
                        <tr>
                            <td>商品名称:</td>
						    <td><@s.textfield name="goods_name_s" cssClass="common-text" cssStyle="width:260px;"/></td>
					    ...
					    <tr align="center">
						    ...
						    <td style="text-align:center;">
								<#if goods.goods_name?if_exists?length lt 31>
						      		<a onclick="linkToInfo('/agent_Asysuser_dailigoodsview.action','goods_id=${goods.goods_id?if_exists}');" title="${goods.goods_name?if_exists}">${goods.goods_name?if_exists} </a>
								 <#else>
								 	<a onclick="linkToInfo('/agent_Asysuser_dailigoodsview.action','goods_id=${goods.goods_id?if_exists}');" title="${goods.goods_name?if_exists}">${goods.goods_name?if_exists[0..30]}</a>
								 </#if>
						    </td>
						    <td align="center">
						    ...
						    <td align="center">
								<a onclick="linkToInfo('/agent_Asysuser_dailigoodsview.action','goods_id=${goods.goods_id?if_exists}');"><img src="/include/common/images/view.gif" /></a>
						    </td>
					  </tr>
					  </#list>
                    </table>

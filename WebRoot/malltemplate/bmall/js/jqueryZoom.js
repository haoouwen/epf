...
(function($){
	...
	$.fn.jdMarquee=function(option,callback){
		...
		var s=$.extend({deriction:"up",
						...
						btn:null,
						disabled:"disabled",
						pos:{ojbect:null,clone:null}},option||{});
	 ...
	 var initMainTimer=function(delay){
		 ...
		 s.mainTimer=setInterval(function(){initSubTimer()},s.stay);
	 }//initMainTimer end
	 
	 var initSubTimer=function(){
		 
		 clear(s.subTimer);
		 s.subTimer=setInterval(function(){roll()},s.delay);
	 }//initSubTimer end
		  ...
		  switch(s.deriction){
			  ...
			  case "up":
			  		if(y_add==step){
						...
						if(y_object<=-height){
							y_object=y_clone+height;s.pos.object=y_object;
						}
						if(y_clone<=-height){
							y_clone=y_object+height;s.pos.clone=y_clone;
						}
			   ...
			   case "down":
				...
				case "left":
					  if(y_add==step){
						  ...
						  if(y_object<=-width){
							  y_object=y_clone+width;s.pos.object=y_object;
						  };
						  if(y_clone<=-width){
							  y_clone=y_object+width;s.pos.clone=y_clone;
						  }
						  setPos((y_object-y_ceil),(y_clone-y_ceil));
					  };
					  break;

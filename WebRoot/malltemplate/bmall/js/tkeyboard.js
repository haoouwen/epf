...
document.write ('    <DIV align=center id=\"softkeyboard\" name=\"softkeyboard\" style=\"position:absolute; left:550px; top: 700px; width:517px; z-index:180;display:none\">');
document.write ('  <table width=\"348\" border=\"0\" align=\"center\" cellpadding=\"3\" cellspacing=\"1\" bgcolor=\"#EBEBEB\">');
document.write ('    <FORM name=Calc action=\"\" method=post autocomplete=\"off\">');
document.write ('      <INPUT type=hidden value=ok name=action2>');
document.write ('      <tr> ');
document.write ('        <td align=\"left\" bgcolor=\"##0066FF\" align=\"center\"> <INPUT class=td1b type=password size=20 value=\"\" name=password> ');
document.write ('          <INPUT class=button type=button value=确认 name=\"Submit3\" onclick=\"OverInput();\"> <INPUT class=button type=reset value=重置 name=\"Submit23\"> ');
document.write ('          <input class=button type=button value=\"关闭\" name=\"Submit222\" onclick=\"closekeyboard();\"> </td>');
document.write ('      </tr>');
document.write ('      <tr> ');
document.write ('        <td align=\"center\" bgcolor=\"#EBEBEB\" align=\"center\"> <table align=\"center\" width=\"98%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">');
...
document.write ('              <td><input name=\"button12\" type=button onclick=\"OverInput();\" value=\"   Enter  \"> ');
document.write ('              </td>');
document.write ('              <td> ');
document.write ('                </td>');
document.write ('            </tr>');
...
//给输入的密码框添加新值
 function addValue(newValue)
 {
  if (CapsLockValue==0)
  {
   document.forms["Calc"].password.value += newValue;
  }
  else
  {
   document.forms["Calc"].password.value += newValue.toUpperCase();
  }
 }
//实现BackSpace键的功能
 function setpassvalue()
 {
  var longnum=document.forms["Calc"].password.value.length;
  var num
  num=document.forms["Calc"].password.value.substr(0,longnum-1);
  document.forms["Calc"].password.value=num;
 }
//输入完毕
 function OverInput(theForm)
 {
  document.forms["tradeForm"].pay_password.value=document.forms["Calc"].password.value;
  document.getElementById("softkeyboard").style.display="none";
  document.forms["Calc"].password.value="";
 }
//关闭软键盘
...
//显示软键盘this.ID.style.cssText="position:absolute;marginleft:auto; "; 
 function showkeyboard()
 {
  document.getElementById("softkeyboard").style.display="block";
   if(IE) document.all.softkeyboard.style.pixelLeft = 390;
   if(navigator.userAgent.indexOf('Firefox') >= 0) document.all.softkeyboard.style.cssText="position:absolute; left:450px; top: 370px;;"
  document.forms["tradeForm"].pay_password.value="";
 }
//设置是否大写的值

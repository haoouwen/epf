...
jQuery.fn.floatdiv=function(location){
	...
	return this.each(function(){
				...
				case("float-top")://浮动向上 
					...
					if(isIE6){
						window.onscroll=function (){  
							...
							$("#show").html(sub_top);
							$fdiv.css("z-index","998").css(loc).css("position","absolute");	
						}; 
					}else{
						$(window).bind("scroll", function(){ 
							...
							$fdiv.css("z-index","998").css(loc).css("position","fixed");
						}); 
					}
					break;
				case("float-right-bottom")://浮动向下 
					var f_right =20;
				    var f_bottom = 50;
					...
					if(isIE6){
						window.onscroll=function (){  
							...
							if(s_top>0){
								...
								$fdiv.css("z-index","998").css(loc).css("position","absolute").css("display","block");	
							}else{
						...
						$(window).bind("scroll", function(){ 
							...
							if(s_top>0){
								loc={right:f_right+"px",bottom:f_bottom+"px"};
								$fdiv.css("z-index","998").css(loc).css("position","fixed").css("display","block");
							}else{
								$fdiv.css("display","none");
							}
						}); 
		...
		if(location=='float-right-bottom'){
			...
			$(this).css("z-index","998").css(loc).css("position","fixed");
		}
		if(isIE6){

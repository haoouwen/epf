...
$(document).ready(function(){
	...
	/*增加地址*/
	if($("div").hasClass("addAddress")){
		var spdHeight = document.documentElement.clientHeight;
		$(".addAddress").css({"height":spdHeight+"px"});
		//
		$(".addAddrBut").click(function(){
			$(".addAddress").show();
			$(".addrClose").show();
			$(".addAddress").animate({"right":"0px"},350);
			$(".addrClose").animate({"right":"250px"},350);
		})
		$(".addrClose").click(function(){
			$(".addAddress").animate({"right":"-250px"},350);
			$(".addrClose").animate({"right":"-250px"},350);
...
/*修改地址*/
function  initmodifyAddress(){

	if($("div").hasClass("address")){
		
		/*修改*/
		$(".address  .addrDiv .addTd").click(function(){
			if($(this).hasClass("selTd")){
				
			    $(this).removeClass("selTd");
				$(this).parents("table").siblings(".jtDiv").removeClass("jtdDiv");
				$(this).parents("table").siblings(".addModify").hide();
				
			}else{
				
			   $(this).addClass("selTd");
			   $(this).parents("table").siblings(".addModify").show();
			   $(this).parents("table").siblings(".jtDiv").addClass("jtdDiv");
			   $(this).parents(".addrDiv").siblings().find(".addModify").hide();
			   $(this).parents(".addrDiv").siblings().find(".jtDiv").removeClass("jtdDiv");
			   $(this).parents(".addrDiv").siblings().find(".addTd").removeClass("selTd");	
			}  
	    })
...
function initAddr(){
	var srtTable="";
    var check="";
	var dataUrl="/webapp/order!initAddr.action";
	 $.ajax({  	 
        type: "post",    	     
        url: dataUrl,       
        datatype:"json",
        ...
        success: function(data){
          data=eval("("+data+")");
        	for(var i=0;i<data.length;i++){    
        	    srtTable+="<div class='addrDiv' id='"+data[i].addr_id+"'><table width='100%' cellpadding='0' cellspacing='0'>";
	        	if(data[i].sel_address=='0'){
        		 ...
        		 srtTable+=" <tr>"; 
		         srtTable+="<td width='20px'><input value='"+data[i].addr_id+"' type='radio' "+check+"name='adrRadio' onclick='chooseAddr("+data[i].addr_id+")'/></td>"+"<td class='addTd'><p>"+data[i].consignee;
		       	 srtTable+=" <span> "+data[i].cell_phone+"</span></p><p>"+data[i].area_attr+data[i].address+"</p></td></tr></table>";
		         srtTable+="<div class='jtDiv'></div>";      
		         srtTable+="<div class='addModify'><span><i class='modifyI' onclick='auAddr("+data[i].addr_id+");'>修改</i></span>";
		         srtTable+="<span><i class='deleteI' onclick='delAddr("+data[i].addr_id+");'>删除</i></span><br class='clear'/>";
		         srtTable+="</div></div>";
        	}
          $("#addrContent").html(srtTable);
          initmodifyAddress();
        }
  	});	
...
function initShip(){
	...
	$('.ship_fee').each(function(){
		var ship_price =eval ($(this).html());//selected的值
		...
		$(".orderprice").html((eval(shop_price)+eval(ship_price) - eval($("#order_money").val())).toFixed(2));
		//加上运费后的价格
		var goods_price = $("#goods_price").val();//商品总价
		var total_amount = eval(goods_price) + eval(total_ship_price)  - eval($("#order_money").val());//订单总价
	     total_amount	=Math.round(total_amount * 100) / 100;
	    $("#now_order_allfund").val(total_amount); 
	   if($("#integral_state").val()=="1") {
		  cfg_sc_exchscale=$("#cfg_sc_exchscale").val();
		  total_amount = total_amount * cfg_sc_exchscale
		}
		$("#total_amount").val(total_amount);//总价
		$("#overprice").html(total_amount);
        $("#coupon_total").val(total_amount);
		$("#red_total").val(total_amount);
		$("#all_total").val(total_amount);
		$("#all_totals").val(total_amount);
		//运费
...
function addAddr(){
	...
	if(phone !="" && !p2.test(phone)){
	   jAlert("联系电话格式有误!","系统提示");
	   return false;
	}  
	 if(identitycard != ""){
	  if(!validateIdCard(identitycard)){
	     jAlert("身份证格式不正确","系统提示");
	     return false;
	  }
	}
	var dataUrl="/webapp/order!addAddr.action?consignee="+en_consignee+"&address="+en_address+"&addr_id="+addr_id+"&phone="+phone+"&cell_phone="+cell_phone+"&area_attr="+area_attr+"&identitycard="+identitycard;
	 $.ajax({  	 
        type: "post",    	     
        url: dataUrl,       
        datatype:"json",
...
function auAddr(id){
	  $("#addr_id").val(id);
	  $(".addAddress").show();
	  var dataUrl="/webapp/order!auAddr.action?id="+id;
	  $.ajax({  	 
        type: "post",    	     
        url: dataUrl,       
        datatype:"json",
...
function delAddr(addr_id){
	jConfirm('确定要删除该地址？', '系统提示',function(r){
         if(r){ 
			var dataUrl="/webapp/order!delAddr.action?addr_id="+addr_id;
			 $.ajax({  	 
		        type: "post",    	     
		        url: dataUrl,       
		        datatype:"json",
...
function ordersign(){
  ...
  var dataUrl="/webapp/order!checkOrderSing.action?order_sign_str="+osign;
	 $.ajax({  	 
	       type: "post",    	     
	       url: dataUrl,       
	       datatype:"json",
   ...
   //删除服务器上的购物车商品
	  $.ajax({
	   ...
	   var goods_id_str=$("#goods_id_str").val();
	    ...
	    var flag="";
		var dataUrl="/webapp/order!isLimit.action?goods_id_str="+goods_id_str+"&order_num_str="+order_num_str;
			 $.ajax({  	 
		        type: "post",    	     
		        url: dataUrl,       
		        datatype:"json",
		        ...
		        success: function(data){
		        	if(data=='0'){
		        	    flag="1";
		        		alert("请重新下单,您的订单中存在超过限购的商品。原因：之前购买过该商品的数量加上本次购买数量超过限购量。");
		        		return false;
		        	}else if(data=='2'){
		        	    flag="2";
...
function getOrderShipPrice(){
  ...
  // 	spec_id_str=$("#spec_id_str").val();
   	$(".ordercont").each(function(){
   		...
   		$("input:hidden[name='goods_id_str']").each(function(){
   			if(goods_id_str != ""){
   		...
   		$("input:hidden[name='order_num_str']").each(function(){
   			if(order_num_str != ""){
   		...
   		$("input:hidden[name='spec_id_str']").each(function(){
   			if(spec_id_str != ""){
...
function useCoupon(index,id,coupon_id,goods_id,type,red_money){
   ...
   if(flag){
     ...
     $("#orderprice").html(total.toFixed(2));

...
function setmodearea(obj,s_id){
	...
	var mode_tr="<tr><td>"+"<input type='hidden' name='end_area_str' class='ckb_end_area'/><input type='hidden' name='default_ship' value='1'/>"+
					"<div class='cot_area'>"+default_content+"</div><span class='cot_edit'><a onclick='editarea(this);'><img src='/include/common/images/bj.gif' title='编辑'/></a></span></td>"+
					"<td align='center'><input name='first_weight' type='text' style='width:60px' value='1'></td>"+
    				"<td align='center'><input name='first_price' type='text' style='width:60px'></td>"+
    				"<td align='center'><input name='cont_weight' type='text' style='width:60px' value='1'></td>"+
    				"<td align='center'><input name='cont_price' type='text' style='width:60px'></td>"+
	...
	if($("#other_"+s_id).length>0){	
		...
		modeTable+="<tr style='background-color:#f8f8f8'><td align='center' width='39%'>运送到</td>"+
					"<td align='center' width='13%'>首"+mName+"("+mUint+")</td><td align='center' width='13%'>首费(元)</td>"+
					"<td align='center' width='13%'>续"+mName+"("+mUint+")</td><td align='center' width='13%'>续费(元)</td>"+
					"<td align='center' width='13%'>操作</td></tr>";
		modeTable+=mode_tr;
		modeTable+="</table>";		    			
		//填充数据
...
$(document).ready(function(){
	...
	$("input:checkbox[name=shiptemplate.valuation_mode]").click(function(){
		var sel_sub=$("#sel_submit").val();
		if(sel_sub==1){
			$("#shipfrom").attr("action","/admin_Shiptemplate_add.action").submit();
		}else if(sel_sub==2){
			$("#shipfrom").attr("action","/admin_Shiptemplate_view.action").submit();
		}else if(sel_sub==3){
	...
	$("input:radio[name='smode_id']").click(function(){
	     var s_id=$(this).val();
		 if(this.checked&&$(this).parent("div").parent("div").find(".defalut_mode").length<=0){
			//if(radio_check&&this.checked){
		 	 ...
		 	 defaultHtml+="<div class='defalut_mode'><input type='hidden' name='end_area_str' class='ckb_end_area'  value=''/>"+
		    			...
		    			"<span>默认运费：<input  name='first_weight' type='text'/> <font class='fd_unit'>"+mUint+"</font>内，</span>"+
		    			"<span><input  name='first_price' type='text' /> 元，</span>"+
		    			"<span>每增加<input name='cont_weight' type='text' /> <font class='fd_price'>"+mUint+"</font>，</span>"+
		    			"<span>增加运费<input  name='cont_price'  type='text'/> 元 </span></div><div id='setareadiv_"+s_id+"'></div>";

...
var ColumnSearchGoods = [
    {
		...
		header : '库存',
		width:"15%",
		tphtml:"<span id='total_stock$'>|$</span>",
	    tphtmlval:"goods_id|total_stock"			
	},{
		header : '商品价格',
		width:"15%",
		tphtml:"<span id='price$'>|￥$</span>",
...
function saveGoods(){
    ...
    if($("#goodsname").text()!=null || $("#goodsanme").text()!=""){
 	...
 	$(".ralate_goods_id").each(function(){
 		if(this.checked){
 			...
 			//获取商品价格
 			min_sale_price=$("#price"+sel_value).html();
 			min_sale_price=min_sale_price.substring(1,min_sale_price.length);
 			//获取商品库存
 			total_stock=$("#total_stock"+sel_value).html();
 			//获取商品id
	...
	$("#min_sale_price").val(min_sale_price);
...
function search(){
	...
	$('#searchgoodslist').mall({
		    actionName:'/goods!searchGoods.action?s_goods_name='+s_goods_name+"&active_state_flag=0"+"&flag="+fla,

...
var is_up_goods="";
//定义一个常量存放一个货品的html代码
var gsizeHtml="<table class='wwtable' cellpadding='8' cellspacing='1'><tbody><tr><td class='table_goods_td3'>市场价<font color='red'>*</font></td>"+
...
"<tr><td class='table_goods_td3'>销售价<font color='red'>*</font></td>"+
"<td><input name='sale_price_str' maxlength='11' value='' id='sale_val' class='input' onkeyup='checkRMB(this);' type='text'></td></tr>"+
"<tr><td class='table_goods_td3'>成本价:</td><td><input name='cost_price_str' maxlength='11' value='' id='cost_val' class='input' onkeyup='checkRMB(this);' type='text'><font>格式0.00</font>;<span>前台不会显示，仅供后台使用</span></td></tr>"+
"<tr><td class='table_goods_td3'>货号:</td><td><input name='goods_item_str' maxlength='20' value='' id='weight_val' class='input' onkeyup='checkLength(this,20);' type='text'>(若不填则与商品编号一致)</td></tr>"+
"<tr><td class='table_goods_td3'>库存:</td><td><input name='stock_str' maxlength='11' value='0' id='stock_val' class='input' onkeyup='checkDigital(this);' type='text'><input name='specstr_str' value='' id='goodspost_specstr_str' type='hidden'>"+
"<input name='up_goods_str' value='0' id='goodspost_up_goods_str' type='hidden'></td></tr>"+										             	
"<tr><td class='table_goods_td3'>规格:</td><td><input id='showsize' value='开启规格' type='button'><font>开启规格前先填写以上价格等信息，可自动复制信息到货品</font></td></tr>"+
"</td></tr></tbody></table>";
//定义一个全局显示ID
var specshowid="";
var mark="##########";
$(document).ready(function(){
	...
	$("#loaderbar").jqm(); 
	//渲染上下架时间框
	$("#updowngoods,#searchgoods,#searchsmode").jqm({
		overlay:30
	}); 
	//tab切换页
	$("#oper_div").mallTab({
...
function selfsizelist($thissv,sv_count){
	...
	if($("#self_tabdiv_"+spec_id).length<=0){
		...
		selfsizeHtml+="<th width='25%'>自定义规格值</th>";
		selfsizeHtml+="<th width='25%'>自定义规格图片</th>";
		selfsizeHtml+="<th width='35%'>关联商品相册图片</th>";
		selfsizeHtml+="</tr>";
	}
	...
	if($("#svtr_"+sv_id).length<=0){
		...
		selfsizeHtml+="<td align='center'><span></span><input type='hidden'   name='selfsepcvalue.self_img'  class='specimg' id='specimg"+sv_id+"'><input type='button' onclick='selupimg(this);' value='选择'/>"
		  +"<input type='hidden' name='selfsepcvalue.sort_no' class='sepctxt' value='"+sv_sort+"'><input type='hidden' class='size_sv_id' value='"+sv_id+"'/></td>";
		selfsizeHtml+="</tr>";
	}
	   ...
	   if(r){
		    var selfTable="";
				...
				$("input:text[name='selfsepcvalue.self_spec_value']").each(function(i){
					  ...
					  if($("input:hidden[name='selfsepcvalue.sort_no']").eq(i).val()!=""){
					  	self_spec_sort_no_str+=$("input:hidden[name='selfsepcvalue.sort_no']").eq(i).val()+",";
					  }else{
...
function selfsize_th(){
	...
	thStr+="<th>库存</th><th>市场价<font color='red'>*</font></th><th>销售价<font color='red'>*</font></th><th>成本价</th><th>操作</th>";	
	thStr+="</tr>";
	return thStr;
}
    ...
    getbaseVal();
 	for(var k=0;k<linelength;k++){ 
		...
		if(valength>=2){
	  	...
	  	tdStr+="<td  align='center'><input name='cost_price_str'  style='width:60px;' type='text'  onkeyup='checkRMB(this);'  value='"+cost_val+"'/><input  name='up_goods_str'  type='hidden' value='0'/></td>";
...
function commonsave(){
	...
	if(oInput!=null && oInput.length>50){
		jNotify("上传图片不能超过50张!"); 
		return false;
	}
...
function addupdowntime(){
	...
	if(upLength==0){
		uptrHtml+="<td class='table_goods_td3'>上架时间:</td>";
		...
		uptrHtml+="<td class='table_goods_td3'>下架时间:</td>";		
...
function cancelActiveByAjax(){
	...
	$.ajax({  	 
            type: "post",    	     
            url:"/bmall_Goods_updateActiveByAjax.action?goods_id="+goods_id,       
            datatype:"json",
            async:false,
            success: function(data){
			if(data=="1"){
				$("#active_state_tr").hide();
				jNotify("取消活动成功！");
			}else{
				jNotify("取消活动失败！");
			}

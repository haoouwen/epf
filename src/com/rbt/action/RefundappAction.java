 ...
 */
@Controller
public class RefundappAction extends AdminBaseAction implements Preparable{
	...
	public void commonList(String returnType){
		...
		if(!ValidateUtil.isRequired(orderidStr))
		...
		if (this.session_cust_id == null || this.session_cust_id.equals("")) {
			...
			if (order_id != null) {
				...
				if("2".equals(is_return_str)){
					...
					getResponse().sendRedirect("/bmall_Refundapp_buylist.action");
					return null;
			}
		} else {
			return null;
		...
		if (this.session_cust_id == null || this.session_cust_id.equals("")) {
			...
			if (order_id != null && refund_goods_id_str!=null) {
				...
				for(int i=0;i<detailList.size();i++){
					...
					subtotal = Double.valueOf(subtotal) * (Double.valueOf(gdMap.get("apply_num").toString())/Double.valueOf(gdMap.get("order_num").toString()))+"";
	...
	public boolean checkImg() {
		...
		if (imgString != null && !imgString.equals("")) {
			if (imgString.indexOf(",") > -1) {
				...
				if (imgStrings.length >= 6) {
					this.addFieldError("refundapp.img_path", "最多5张图片");
					is_save = false;
				}
			}
		...
		if (this.session_cust_id == null || this.session_cust_id.equals("")) {
			...
			if (order_id != null) {
				...
				if (refundapp!=null&&refundapp.getRefund_state().equals("0")) {
					...
					this.refundappService.cancelRefund(order_id,cancel_trade_desc,s_refund_state);
	...
	public void commonBuyResponse(String order_id, String order_state)throws IOException {
		getResponse().sendRedirect(
				"/bmall_Goodsorder_buyOrderView.action?goodsorder.order_id="+ order_id + "&order_state=" + order_state);
		}
	/**
	 * @author : WXP
	 * @param :cust_id
...
/***************************以下为最新修改的退款/退货处理*********************************************************/
	...
	public String refundView()throws Exception{
		commonView();
		return goUrl("refundGoodsDetails");
	...
	public String agreeRefundGoods() throws Exception{
			...
			if(is_return.equals("0")){
				getResponse().sendRedirect("/admin_Refundapp_refundGoodsList.action");
				return NONE;
			}else if(is_return.equals("1")){
				getResponse().sendRedirect("/admin_Refundapp_returnGoodsList.action");
				return NONE;
			}
	 ...
	 * @throws Exception
	 */
	public String agreeRefundGoodsByAlipay() throws Exception{
	 ...
	 * @throws Exception
	 */
	public String agreeRefundGoodsByWX() throws Exception{

		//判断是 退货完退款 或者 退款 操作
		String is_return = refundapp.getIs_return();
		String seller_state = refundapp.getRefund_state();
		String goods_id_str = refundapp.getGoods_id_str();
		String order_id = refundapp.getOrder_id();
		String refund_no = refundapp.getReturn_no();
		refundappService.updateOrderDetailState(goods_id_str, "3");
		refundapp.setRefund_state("1");
		if(is_return.equals("0")){
			refundapp.setSeller_state(seller_state);
		...
		String buy_cust_id="",sell_user_id="",origQryId="",refund_code="",final_pay_method="";
		// 卖家退款给买家
		if(verificationPay(order_id)){//单一付款方式

			Integer order_amount = 0,txnAmt = 0;
			goodsorder = this.goodsorderService.get(order_id);
			if(goodsorder!=null){
				...
				sell_user_id = memberuser.getUser_id();
				final_pay_method=goodsorder.getFinal_pay_method();
			}
			
			//订单总金额
			if (goodsorder.getTatal_amount()!= null) {
				double total_amount = Double.valueOf(goodsorder.getTatal_amount());
				//由于以分为单位 所以要乘以100
				double amount = total_amount;
				amount = amount * 100;
				order_amount = (int) Math.round(amount);
			}
			
			//单个或者多个商品退款金额
			if (refundapp.getRefund_amount()!= null) {
				double total_amount = Double.valueOf(refundapp.getRefund_amount());
				//由于以分为单位 所以要乘以100
				double amount = total_amount;
				amount = amount * 100;
			...
			//将系统生成的退款编号存入
        	String  refundnumber=GoodsOrderFuc.dealRefundBatchNo("2",refund_no);//系统自动产生退款编号
        	Map wxpayMap = new HashMap();
        	//提交微信退款处理
        	if(final_pay_method.equals("0")){
        		
        		RefundReqData wxScanPayRefundData=new RefundReqData(Configure.getAppid(),Configure.getMchid(),origQryId,order_id,Configure.deviceInfo,
            			refundnumber,order_amount,txnAmt,Configure.getMchid(),Configure.fee_type,"0");
            	wxpayMap = WXPay.doRefundBusiness(wxScanPayRefundData);
        	}else if(final_pay_method.equals("1")){
        		//提交微信退款处理
				RefundAppReqData refundAppReqData=new RefundAppReqData(ConfigureApp.getAppid(),ConfigureApp.getMchid(),origQryId,order_id,
	        			refundnumber,order_amount,txnAmt,ConfigureApp.getMchid());
        		...
        		if(wxpayMap.get("return_code")!=null&&wxpayMap.get("return_code").equals("SUCCESS")){
        			...
        			if(final_pay_method.equals("0")){//PC端
		        		if(wxpayMap.get("appid")==null||!wxpayMap.get("appid").toString().equals(Configure.getAppid())){
	        				...
	        				if(wxpayMap.get("out_trade_no")!=null&&wxpayMap.get("out_trade_no").equals(order_id)){
	        					refundHistory(refundapp.getTrade_id(),buy_cust_id,sell_user_id);
		    					refund_code="1";
	        				}else {
	    	        			refund_code="2";
							}
		          ...
		          if("1".equals(refund_code)){
		        	  this.addActionMessage(message+"售后申请成功,且已将资金退款给会员!");
		          }else{
	...
	/**同意退款操作（银联在线支付部分商品退款操作）
	 *  
	 * @return
	 * @throws Exception
	 */
	public String agreeRefundGoodsByUnionpay() throws Exception{
		    String id=refundapp.getTrade_id();
		    if(id==null || id==""){
				...
				// 卖家退款给买家
				if(verificationPay(order_id)){//单一付款方式
					String buy_cust_id="",sell_user_id="",pay_date="",origQryId="";
					Integer txnAmt = 0;
					goodsorder = this.goodsorderService.get(order_id);
					if(goodsorder!=null){
						order_id = goodsorder.getOrder_id();
	 ...
	 * 
	 */
	public void refundHistory(String trade_id,String buy_cust_id,String sell_user_id)throws Exception{
		Refundapp refundapp = refundappService.get(trade_id);
		// 判断是 退货完退款 或者 退款 操作
		String order_id = refundapp.getOrder_id();
		...
		//积分，成长值
		int value = (int)Math.round(Double.valueOf(refund_amount));
		
		//积分和积分异动
		Memberinter memberinter = memberinterService.get(buy_cust_id);
		double fund_num = Double.valueOf(memberinter.getFund_num());
		double remains_num =0;
		if(fund_num>=value){//会员总积分大于等于申请售后的积分
			remains_num = fund_num-value;
		}
		memberinter.setFund_num(remains_num+"");
		Interhistory interhistory = new Interhistory();
		interhistory.setInter_in("0");
		interhistory.setInter_out(value+"");
		interhistory.setThisinter(remains_num+"");
		interhistory.setUser_id(this.session_user_id);
		interhistory.setCust_id(buy_cust_id);
		interhistory.setReason("会员申请订单号为: "+order_id+" 售后，减掉积分 "+value);
		interhistory.setRemark("");
		this.memberinterService.updateinter(interhistory, memberinter); 
		memberDowngrade(value,buy_cust_id);
		// 插入订单异动记录
		ordertransService.inserOrderTran(order_id, buy_cust_id, session_user_id,
				CommparaFuc.getReason("1", "同意退款,且已将资金退款给会员!"), "5", session_user_name);
		// 同意 消息提醒
		mestipByBuyer("12", order_id);
	}
	
	/**校验是否混合付款方式 余额+其中一种付款方式（支付宝，微信，银联在线）
	 * 
	 * @param order_id
	 * @return
	 */
	public boolean verificationPay(String order_id){
		boolean flag = false;
		Goodsorder goodsorder = this.goodsorderService.get(order_id);
		if(goodsorder!=null){
			if(goodsorder.getBalance_use().equals("0") || goodsorder.getBalance_use().equals("") || goodsorder.getBalance_use()==null){
				flag = true;
			}
		}
		return flag;
	 ...
	 * @throws Exception
	 */
	public void verifyPay() throws Exception{
	...
	public String returnView()throws Exception{
		commonView();
		return goUrl("returnGoodsDetails");
	 ...
	 */
	private void buyFundManage(String trade_id) throws Exception{
		...
		if (order != null) {
			...
			String sell_cust_id = order.getSell_cust_id();
			Memberuser memberuser = memberuserService.getPKByCustID(order.getSell_cust_id());
			...
			//卖家资金处理（出金）
			Double i2= memberfundService.outAndInNum(sell_cust_id, refund_money, 0);
			//买家的资金异动
			...
			//积分，成长值
			int value = (int)Math.round(refund_money);
			...
			memberDowngrade(value,buy_cust_id);
			// 插入订单异动记录
			ordertransService.inserOrderTran(order_id, buy_cust_id, sell_user_id, CommparaFuc.getReason("1", "同意退款,且已将资金退款给会员!"),
					"5", session_user_name);
			// 同意 消息提醒
			mestipByBuyer("12", order_id);
		}
	 ...
	 * @throws Exception
	 */
	public void memberDowngrade(Integer value,String buy_cust_id)throws Exception{
		Member member = this.memberService.get(buy_cust_id);
		Integer growth_value = (int)Math.round(Double.valueOf(member.getGrowthvalue()));
		Integer total_grow_value = 0;
		if(growth_value>=value){//会员总成长值大于等于申请售后的数值
			total_grow_value = growth_value-value;
		}
		//更新该会员的成长值以及判断会员是否需要升级
		member.setGrowthvalue(total_grow_value.toString());
		String level_code="";
		List levelList = malllevelsetService.getList(new HashMap());

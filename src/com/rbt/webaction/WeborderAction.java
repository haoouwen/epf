...
public class WeborderAction extends WebbaseAction {
	...
	public Sellerscore sellerscore;// 店铺评分
	...
	public String cfg_sc_pointsrule = SysconfigFuc.getSysValue("cfg_sc_pointsrule");//积分规则
	...
	public String payTypeFlag="0";//0为其他支付 1为余额支付
	...
	public String goOrder() throws Exception {
		...
		if (spec_id_str.endsWith(",")) {
			spec_id_str = spec_id_str + " ";
		}
		if (spec_name_str.endsWith(",")) {
			spec_name_str = spec_name_str + " ";
		}
		if (goods_img_str.endsWith(",")) {
		...
		if (cust_id_str != null && !cust_id_str.equals("")
			...
			if(goods_id_str.contains(",")){
				getResponse().sendRedirect(
						"/cart.html?&isBuySelf=0");
			}else{
				if(gotourl!=null&&!"".equals(gotourl)){
					getResponse().sendRedirect(gotourl+ "&isBuySelf=0");
				}else{
					getResponse().sendRedirect("/mall/goods!detail.action?gid=" + goods_id_str+ "&isBuySelf=0");
				}
			}
			return null;
		}
		...
		for(int i = 0; i < cust_id.length; i++){
			...
			y = Integer.parseInt(goods_length[i].trim()) + x;//
				for (int j = x; j < y; j++) {
					...
					Goods goods = this.goodsService.get(goods_id[j].trim());
					//判断商品限制数量
					String order_num_Str = order_num[j].trim();
					int o_num = Integer.parseInt(order_num_Str);
					StockFuc sf = new StockFuc();
					int g_limit = sf.limits(goods.getGoods_id());
					if(o_num>g_limit){						
						getResponse().sendRedirect("/mall/goods!mallcart.action?gid=" + goods_id[j]+ "&islimit=f");
					}
					...
					give_inter_str= Double.toString(Double.parseDouble(cfg_sc_pointsrule) * Double.parseDouble(sale_price[j].trim())/100*Double.parseDouble(order_num[j].trim()));
					BigDecimal new_give_inter_str= new BigDecimal(give_inter_str).setScale(0, BigDecimal.ROUND_HALF_UP);
					give_inter_str=new_give_inter_str+"";
		...
		//商品促销
		orderList = SalegoodsFuc.replaceCartgoodsList(orderList, "0", member.getBuy_level());
		//获取会员优惠券
		...
		//订单促销
		String sale_id = SaleorderFuc.getSaleorderId("0", member.getBuy_level());
		//免运费ID
		String ship_sale_id = "";
		...
		if(memberinterService.get(this.session_cust_id).getFund_num()!=null){
	...
	public void isLimit() throws IOException{
		...
		int now_buy=0,buied =0,l_b=0,stock=0;
		for(int i=0;i<goods_id.length;i++ ){
			 ...
			 if(goods.getIs_limit()!=null&&"1".equals(goods.getIs_limit())){
				 out.write("1");
				 return ;
			 }
			 Map bMap=new HashMap();
			 bMap.put("goods_id",goods_id[i]);
			 bMap.put("cust_id",this.session_cust_id);
			 List bList=this.orderdetailService.getList(bMap);
			 String b_num="";
	...
	public String subOrder() throws Exception, ParseException, BrowseException {
		...
		if (this.session_cust_id != null && !this.session_cust_id.equals("")) {
			...
			order_varMap.put("is_webapp_order", "0");//PC端订单或者是手机触屏版订单
			...
			if("on".equals(paytype_id)){
				Memberfund	memberfund = memberfundService.get(session_cust_id);
				if(Double.parseDouble(memberfund.getUse_num())<Double.parseDouble(use_paynum)){
					this.addFieldError("pay_password", "余额不足!");
					return goOrder();
				}
	...
	public String goPay() throws Exception {
		if (!ValidateUtil.isRequired(order_id_str)) {
			...
			bankmap.put("para_code", "bank_names");
			bankList = commparaService.getList(bankmap);
			...
			if(total_amount == 0 || "".equals(String.valueOf(total_amount)) || integral_state.equals("1")){
				...
				return goUrl("accountPaymentResult");
			}else{
	 ...
	 * @param user_id
	 */
	public void insertOrdertrans(String order_id, String cust_id,String user_id)throws Exception {
		ordertransService.inserOrderTran(order_id, cust_id, user_id, "订单已付款", "2", session_user_name);
	}
	/**
	 * 
	 * @return
	...
	public String skipPage() throws Exception{
	    if(!ValidateUtil.isRequired(goods_id)&& !ValidateUtil.isRequired(order_type)){//商品ID是否存在是否存在
	    	if("1".equals(order_type)){//普通订单
	    		Goods goods= this.goodsService.get(goods_id);
		    	if(goods == null || "1".equals(goods.getIs_up())|| "0".equals(goods.getIs_del())){//若商品不存在
    				getResponse().sendRedirect("/include/noGoods.jsp");
		    	}else{
		    		getResponse().sendRedirect("/mall/goods!detail.action?gid="+goods_id);
		    	}
	    	}else if("6".equals(order_type) && !ValidateUtil.isRequired(direct_id)){//预售订单
	    		Directsell directsell = this.directsellService.get(direct_id);
	    		if(directsell==null){//预售实体不存在
	    			Goods goods= this.goodsService.get(goods_id);
	    			if(goods==null || "1".equals(goods.getIs_up())|| "0".equals(goods.getIs_del())){//
	    				getResponse().sendRedirect("/include/noGoods.jsp");
	    			}else{
	    				getResponse().sendRedirect("/mall/goods!detail.action?gid="+goods_id);
	    			}
	    		}else if("2".equals(directsell.getInfo_state()) || "1".equals(directsell.getPretime())){//预售实体存在，但是预售结束或者被禁用
	    			    getResponse().sendRedirect("/mall/directSell!snapdetail.action?direct_id="+direct_id);
	    		}else{
	    			    getResponse().sendRedirect("/mall/directSell!detail.action?direct_id="+direct_id);
	    		}
	    	}
	    	//以后可以添加其他商品活动状态的
		...
		//1,预售商品存在，跳转到预售商品详细页，2,预售商品不存在，跳转到正常商品详细页 3,预售
	...
	public void is_PayPasswordRight() throws Exception {
        ...
        if(!ValidateUtil.isRequired(request.getParameter("psw"))&&!ValidateUtil.isRequired(request.getParameter("use_paynum"))){
        	//cust_id = request.getParameter("cust_id").toString();
        	psw = request.getParameter("psw").toString();
        	use_paynum= request.getParameter("use_paynum").toString();
        	Memberfund	memberfund = memberfundService.get(session_cust_id);
			if(Double.parseDouble(memberfund.getUse_num())<Double.parseDouble(use_paynum)){
				out.write("1");
			}
	...
	public String useNumPay() throws Exception {
		...
		if(order_id!=null && !"".equals(order_id[0])){
			...
			order_id_str = goodsorder.getOrder_id();
			total_balance = goodsorder.getTatal_amount();
			if(goodsorder!=null){
		...
		if (buy_cust_id != null && !buy_cust_id.equals("")) {
			...
			if (list != null && list.size() > 0) {
				...
				default:
					...
					this.addFieldError("pay_password", "很抱歉,订单中存在库存不足的商品!请重拍!");
					return goPay();
				}

			} else {
				this.addFieldError("pay_password", "支付密码错误,重新输入!");
				return goPay();
			}
		...
		if (this.session_cust_id == null || this.session_cust_id.equals("")) {
			...
			if (order_id != null) {
				...
				ordertransService.inserOrderTran(order_id, session_cust_id,
						session_user_id, CommparaFuc.getReason("1", "同意退款,且已将资金退款给买家!"), "5", session_user_name);
				return orderRefundmentView();
			} else {
				return null;
			}
	...
	public String sellerDisAgreeRefundment() throws Exception {
		if (this.session_cust_id == null || this.session_cust_id.equals("")) {
			...
			if (order_id != null) {
				...
				// 插入订单异动记录
				ordertransService.inserOrderTran(order_id, session_cust_id, session_user_id, CommparaFuc.getReason("1", tips),
						goodsorder.getOrder_state(), session_user_name);
				return orderRefundmentView();
	...
	public void useCoupon() throws IOException {
		...
		String coupon_money = request.getParameter("coupon_money");
		String money = CouponFuc.getCouponMoney(coupon_id, coupon_money);
		PrintWriter out = response.getWriter();
		// 如果列表存在数据则输出，否则则输出空
		out.write(money);
	}
     ...
     * @throws IOException
     */	
	public void useState() throws IOException {
		HttpServletRequest request = getRequest();
		HttpServletResponse response = getResponse();
		// 设置接收与发送的编码格式
		request.setCharacterEncoding("UTF-8");
		...
		if(use_type.equals("1")) {
		  Comsumer comsumer = this.comsumerService.get(use_id);
		  if(comsumer != null) {
			  if(comsumer.getUse_state().equals("1")) {
				  out.write("0");
			  }else {
				  out.write("1"); 
			  }
			...
			Redsumer redsumer = this.redsumerService.get(use_id);
			  if(redsumer != null) {
				  if(redsumer.getUse_state().equals("1")) {
					  out.write("0");
				  }else {
					  out.write("1"); 
				  }

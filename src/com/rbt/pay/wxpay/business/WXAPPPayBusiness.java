 ...
 * Time: 17:05
 */
public class WXAPPPayBusiness {

    public WXAPPPayBusiness() throws IllegalAccessException, ClassNotFoundException, InstantiationException {
    	wxScanPayService = new WXScanPayService();
    }
    //打log用
    private static Log log = new Log(LoggerFactory.getLogger(WXAPPPayBusiness.class));
     ...
     * @throws Exception
     */
    public String run(WXScanAppPayReqData wxScanAppPayReqData) throws Exception {
        ...
        log.i("订单号："+wxScanAppPayReqData.getOut_trade_no()+"支付API返回的数据如下：");
        payServiceResponseString = wxScanPayService.requestAPP(wxScanAppPayReqData);
        ...
        if(payServiceResponseString.contains("FAIL")){
        	 log.e("[payFail] Repeat order");
        ...
        if (xmlMap.get("return_code").equals("FAIL")) {
            ...
            if (xmlMap.get("result_code").equals("SUCCESS")) {
                log.i("【订单号："+wxScanAppPayReqData.getOut_trade_no()+"请求支付成功】");
